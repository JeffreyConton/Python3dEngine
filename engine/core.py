...
class Core:
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.camera_position = np.array([0.0, 1.0, 0.0], dtype=np.float32)
        self.camera_rotation = np.array([0.0, 0.0], dtype=np.float32)
    ...
    def check_collision(self):
        if self.terrain:
            ...
            if self.camera_position[1] < terrain_height + 1.0:
